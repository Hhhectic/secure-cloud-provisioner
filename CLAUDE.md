...
- Azure is read-only. `azure_crud.py` has working create functions and wiring
  ...
  subscription, so the Azure findings are tested logic rather than measured
  behaviour. That, not the SDK, is what is missing: the SDK is installed now.
...
- **`main.py` is still a second application.** `backend/` is one program
  serving both clouds; the Azure app at the repository root is not part of it
  and shares nothing but the directory. Nothing depends on it any more, so the
  open question is whether it is deleted or kept as the Azure-only deployment
  `/ui` cannot be — see *The two halves are merged*. The two entries that used
  to sit here, saying the halves had never met and that the frontend covered
  AWS only, were made false by that merge and by Azure becoming a
  `ResourceType`; the page grows Azure tabs without being told they exist.
...
2. **Finish the Azure half.** Decided and half done: it is registered and
   ...
   is the same block of work every AWS type already has. The SDK is installed
   now and `/ui` reaches Azure, so what is left is credentials and a run:
   nothing in `az/` has yet touched a real subscription, and until it does the
   Azure findings are tested logic rather than measured behaviour. That is the
   one thing blocking the fix actions, which `az/storage.py` declines to offer
