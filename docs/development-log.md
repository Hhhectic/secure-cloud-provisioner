...
This is the working log the project was built from — a running record of design
decisions, things that were tried, and things that turned out to be wrong. It
was written for the people building the tool, not as documentation for people
using it, and it reads that way.

**It is not a reference and it contradicts itself in places.** Sections describe
the repository as it was at the time they were written; several say so about
themselves. Where this file and the code disagree, the code is right. Start with
[`README.md`](../README.md) instead — this is here because the reasoning behind
a lot of the code is only recorded in one place, and that place is this file.

Two things it is genuinely useful for:

- **Where moto disagreed with real AWS.** Several behaviours were correct
  against the in-memory fake and wrong against the live API. Each one is written
  down here with what the difference was.
- **Why the seams are where they are.** The provider-agnostic warning contract,
  the resource registry, and the decision to keep `scanner/` free of any cloud
  SDK all have their arguments recorded here rather than in the code.
...
the shared team account, including instances and the whole bastion blueprint.
